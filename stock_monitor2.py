...
def quoter(args):
    ...
    print(Fore.YELLOW + Style.BRIGHT + f"{last_datetime} | Ticker: {ticker_symbol} | Low: {float(last_day_low_price):.{dec}f} | High: {float(last_day_high_price):.{dec}f} |"
        f" Open: {float(last_day_open_price):.{dec}f} | Volume: {float(last_day_volume):.{dec}f} | Close: {float(last_day_close_price):.{dec}f}" + Fore.RESET + Style.RESET_ALL)
    ...
    while not stop:
        ...
        if args.color:
            line_color = Fore.BLUE
            #print(type(last_close_price[0]))
            if (last_close_price)[0] == prev_value or prev_value == "":
                color = Fore.YELLOW
            elif (last_close_price)[0] > prev_value:
                color = Fore.GREEN
            else:
                color = Fore.RED
        else:
        ...
        diference = (last_close_price - last_day_close_price)[0]
        #print((diference)[0])
        percentage = (diference / last_day_close_price) * 100
        #print(percentage)
        ...
        print(line_color + Style.BRIGHT + f"{current_datetime} | Ticker: {ticker_symbol} | Low: {float(last_low_price):.{dec}f} | High: {float(last_high_price):.{dec}f} | Open: {float(last_open_price):.{dec}f} |"
            f" Volume: {float(last_volume):.{dec}f} | Close: " + color + f"{float(last_close_price):.{dec}f}    "
            + diference_color + f"{float(diference):.{dec}f} ({float(percentage):.{dec}f}%)" + Fore.RESET + Style.RESET_ALL)
